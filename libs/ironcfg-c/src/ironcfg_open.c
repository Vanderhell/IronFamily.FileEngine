...
ironcfg_error_t ironcfg_get_root(const ironcfg_view_t *view,
    ...
    if (view->header.data_offset > view->buffer_size ||
        view->header.data_size > view->buffer_size - view->header.data_offset) {
        error.code = IRONCFG_BOUNDS_VIOLATION;
        error.offset = view->header.data_offset;
        return error;
    }

    *out_data = view->buffer + view->header.data_offset;
    *out_size = view->header.data_size;
    return error;
}
...
ironcfg_error_t ironcfg_get_schema(const ironcfg_view_t *view,
    ...
    if (view->header.schema_offset > view->buffer_size ||
        view->header.schema_size > view->buffer_size - view->header.schema_offset) {
        error.code = IRONCFG_BOUNDS_VIOLATION;
        error.offset = view->header.schema_offset;
        return error;
    }

    *out_data = view->buffer + view->header.schema_offset;
    *out_size = view->header.schema_size;
    return error;
}
...
ironcfg_error_t ironcfg_get_string_pool(const ironcfg_view_t *view,
    ...
    if (view->header.string_pool_offset > view->buffer_size ||
        view->header.string_pool_size > view->buffer_size - view->header.string_pool_offset) {
        error.code = IRONCFG_BOUNDS_VIOLATION;
        error.offset = view->header.string_pool_offset;
        return error;
    }

    *out_data = view->buffer + view->header.string_pool_offset;
    *out_size = view->header.string_pool_size;
    return error;
}
