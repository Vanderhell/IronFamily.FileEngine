...
/* Test: strict validation detects CRC32 corruption */
static bool test_validate_strict_crc32_detects_corruption(void) {
    uint8_t buf[1024];
    ...
    ironcfg_error_t strict_ok = ironcfg_validate_strict(buf, encoded_size);
    if (strict_ok.code != IRONCFG_OK) {
        return false;
    }

    memcpy(corrupted, buf, encoded_size);

    uint32_t crc_offset = ((uint32_t)buf[36]) |
                          (((uint32_t)buf[37]) << 8) |
                          (((uint32_t)buf[38]) << 16) |
                          (((uint32_t)buf[39]) << 24);
    if (crc_offset == 0 || crc_offset >= encoded_size) {
        return false;
    }

    /* Flip one byte in protected region (before CRC field). */
    corrupted[crc_offset - 1] ^= 0x01;

    ironcfg_error_t strict_bad = ironcfg_validate_strict(corrupted, encoded_size);
    if (strict_bad.code == IRONCFG_CRC32_MISMATCH) {
        return true;
    }
...
int main(void) {
    ...
    test_fn tests[] = {
        ...
        test_validate_strict_crc32_detects_corruption,
