/*
 * EXEC_07 PHASE 5: Native C INCREMENTAL Vector Lifecycle Test
 *
...
#define VECTORS_BASE_PATH "incremental_vectors"
...
/* File reader callback for dual I/O on w+b file handle */
...
static void test_vector(const VectorMeta* meta)
{
    ...
    /* Create output file in w+b mode for dual I/O (read+write) */
    ...
    /* Create writer using file_writer_ctx_t struct */
    ...
    /* Create reader from same file handle (w+b allows both read and write) */
    ...
    {
        /* Seek to beginning for comparison (same FILE* handle) */
